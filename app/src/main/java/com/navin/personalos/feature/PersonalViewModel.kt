...
@HiltViewModel
class PersonalViewModel @Inject constructor(val repository: PersonalRepository,val preferences: PreferenceStore,val reminders: com.navin.personalos.core.reminders.ReminderService): ViewModel() {
    ...
    fun act(message: String = "Saved", block: suspend () -> Unit) {
        ...
        viewModelScope.launch {
            try { block();reminders.reconcile();if(message.isNotBlank()) events.send(message) }
